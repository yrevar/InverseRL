...
geolocator = Nominatim(user_agent="yrevar@gatech.edu")
...
def filter_region_points(data, address_query, lat_span_miles,
    ...
    return data[(data[lat_col] > lat_min) & (data[lat_col] < lat_max) &
                (data[lng_col] > long_min) & (data[lng_col] < long_max)], \
           dict(lat_min=lat_min, lat_max=lat_max, long_min=long_min, long_max=long_max)
...
def df_crop_trips(data, lat, lng, lat_span_miles, lng_span_miles,
                  lat_col="latitude", lng_col="longitude", trip_col="trip_id"):
